  ...
  try {
    ...
    try {
      ...
      if (!homepageContent || homepageContent.length === 0) {
        throw new Error('Homepage returned empty content')
      }
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error'
      console.error(`[Link Crawler] Failed to fetch homepage for ${baseUrl.toString()}:`, errorMessage)
      
      // Provide more specific error messages
      if (errorMessage.includes('timeout') || errorMessage.includes('AbortError')) {
        return NextResponse.json({
          error: 'Request timeout',
          message: 'The website took too long to respond. Please try again or check if the site is accessible.'
        }, { status: 504 })
      }
      ...
      if (errorMessage.includes('ENOTFOUND') || errorMessage.includes('getaddrinfo')) {
        return NextResponse.json({
          ...
          message: 'The domain could not be resolved. Please check the URL and try again.'
        }, { status: 404 })
      }
